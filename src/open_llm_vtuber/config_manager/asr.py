...
class OpenAIAPIASRConfig(I18nMixin):
    """Configuration for OpenAI API ASR."""
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "api_key": Description(
            en="API key for OpenAI API ASR", zh="OpenAI API ASR 的 API 密钥"
        ),
        "base_url": Description(
            en="Base URL for OpenAI-compatible API",
            zh="OpenAI 兼容 API 的 base URL",
        ),
        "model": Description(
            en="ASR model name (e.g., whisper-1)",
            zh="ASR 模型名称（如 whisper-1）",
        ),
        "lang": Description(
        ...
        "prompt": Description(
            en="Optional prompt to guide transcription",
            zh="用于引导转录的可选提示词",
        ),
    }
...
class ASRConfig(I18nMixin):
    ...
    openai_api_asr: OpenAIAPIASRConfig | None = Field(None, alias="openai_api_asr")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "openai_api_asr": Description(
            en="Configuration for OpenAI API ASR", zh="OpenAI API ASR 配置"
        ),
    }
    ...
    def check_asr_config(cls, values: "ASRConfig", info: ValidationInfo):
        ...
        elif asr_model == "openai_api_asr" and values.openai_api_asr is not None:
            values.openai_api_asr.model_validate(values.openai_api_asr.model_dump())
