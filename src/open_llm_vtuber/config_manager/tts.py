...
class DashScopeTTSConfig(I18nMixin):
    """Configuration for DashScope TTS."""

    api_key: str = Field(..., alias="api_key")
    base_url: str | None = Field(None, alias="base_url")
    model: str = Field("qwen3-tts-flash", alias="model")
    voice: str = Field("Cherry", alias="voice")
    language_type: str | None = Field(None, alias="language_type")
    stream: bool = Field(False, alias="stream")
    file_extension: Literal["mp3", "wav"] = Field("wav", alias="file_extension")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "api_key": Description(
            en="API key for DashScope TTS service", zh="DashScope TTS 服务的 API 密钥"
        ),
        "base_url": Description(
            en="Base URL for DashScope API", zh="DashScope API 的基础 URL"
        ),
        "model": Description(
            en="DashScope TTS model name", zh="DashScope TTS 模型名称"
        ),
        "voice": Description(
            en="Voice name to use for DashScope TTS", zh="DashScope TTS 使用的语音名称"
        ),
        "language_type": Description(en="Optional language hint", zh="可选语言提示"),
        "stream": Description(en="Enable streaming mode", zh="启用流式模式"),
        "file_extension": Description(
            en="Audio file format (mp3 or wav)", zh="音频文件格式（mp3 或 wav）"
        ),
    }
...
class TTSConfig(I18nMixin):
    ...
    dashscope_tts: DashScopeTTSConfig | None = Field(None, alias="dashscope_tts")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "dashscope_tts": Description(
            en="Configuration for DashScope TTS", zh="DashScope TTS 配置"
        ),
    ...
    def check_tts_config(cls, values: "TTSConfig", info: ValidationInfo):
        ...
        elif tts_model == "dashscope_tts" and values.dashscope_tts is not None:
            values.dashscope_tts.model_validate(values.dashscope_tts.model_dump())
