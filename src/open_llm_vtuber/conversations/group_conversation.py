...
from ..emotion_learning import update_emotion_memory
    ...
    if full_response:
        ...
        for message in reversed(new_messages):
            if message.startswith("Human:"):
                latest_user_text = message.split("Human:", 1)[1].strip()
                break
        update_emotion_memory(
            conf_uid=context.character_config.conf_uid,
            config=context.character_config.emotion_learning_config,
