...
const sendCaptureBtn = document.getElementById('sendCapture');
...
captureScreenBtn.addEventListener('click', async () => {
    if (!captureScreenBtn || !sendCaptureBtn) {
        return;
    }
    visionStatus.textContent = 'Requesting screen capture permission...';
    ...
    try {
        ...
        const { videoWidth, videoHeight } = captureVideo;
        captureCanvas.width = videoWidth;
        captureCanvas.height = videoHeight;
        const ctx = captureCanvas.getContext('2d');
        ctx.drawImage(captureVideo, 0, 0, videoWidth, videoHeight);
        const dataUrl = captureCanvas.toDataURL('image/png');
        capturedImageData = dataUrl.split(',')[1];
        capturePreview.src = dataUrl;
        capturePreview.style.display = 'block';
        visionStatus.textContent = 'Capture ready. You can send it now.';
        visionStatus.className = 'status success';
        sendCaptureBtn.disabled = false;
        stream.getTracks().forEach(track => track.stop());
    } catch (error) {
        visionStatus.textContent = 'Screen capture failed: ' + error.message;
...
sendCaptureBtn.addEventListener('click', () => {
    if (!capturedImageData) {
        visionStatus.textContent = 'No capture available. Capture screen first.';
        visionStatus.className = 'status error';
        return;
    }
    if (!ws || ws.readyState !== WebSocket.OPEN) {
        visionStatus.textContent = 'WebSocket not connected. Retry in a moment.';
        visionStatus.className = 'status error';
        return;
    }
    ...
    const payload = {
        type: 'text-input',
        text: promptText || 'Please describe the screen.',
        images: [
            {
                source: 'screen',
    ...
    visionStatus.textContent = 'Capture sent to assistant.';
    visionStatus.className = 'status success';
});
...
function updateVisionAvailability() {
    if (!captureScreenBtn || !sendCaptureBtn) {
        return;
    }
    if (!isVisionSupported()) {
        ...
        sendCaptureBtn.disabled = true;
        visionStatus.textContent = 'Screen capture is not supported in this browser.';
        visionStatus.className = 'status error';
    }
}

updateVisionAvailability();
